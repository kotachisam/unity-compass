...
export function scanRepoFiles(articles: ArticleStub[]): RepoFile[] {
  ...
  const dirs = [
    ...SCAN_DIRS.map((d) => ({ root: REPO_ROOT, full: path.join(REPO_ROOT, d) })),
...
export function getSourceReferences(articles: ArticleStub[]): Record<string, RepoFile[]> {
...
export function getSourceReferencesForArticle(slug: string, articles: ArticleStub[]): RepoFile[] {
