...
Working concept: **structural equilibrium** (provisional). Nash equilibrium generalised to partial orderings over structural position-vectors. The strategy space is replaced by a configuration space. The payoff function is replaced by a multi-dimensional structural assessment that doesn't require cardinal utility, only a partial ordering over position-vectors. The formal novelty is not the equilibrium concept (no-unilateral-deviation is inherited from Nash) but the space it operates in and the weaker mathematical object it requires. A partial ordering is strictly weaker than a cardinal payoff function. The thesis question is whether this weaker object produces stability results that the stronger one can't.

The candidate structural axes (risk exposure, entanglement density, constraint burden, procedural commitment strength) are empirically motivated from recurring features of observed coordination-without-convergence cases (AUKUS, Montreal Protocol, IETF, restorative justice). The set is open, not closed. The formal programme tests each candidate for sufficiency (does it generate stability?) and necessity (does removing it break stability?). Additional axes may be identified as the empirical base expands. The framework accommodates this by design rather than requiring a fixed set.

**Parity in the structural domain:** two configurations exhibit parity when each agent, from their own situated perspective, can assess both configurations on structural grounds (the objective structure is measurable) but neither configuration dominates (each is better on different axes). Subjectivity enters in the weighting of axes, not in the measurement. Parity emerges from the interaction between objective structure and subjective weighting: the structure doesn't determine a ranking, and different weightings produce different orderings. This grounds Chang's parity concept in the coordination space without requiring it to be a property of either the objective structure or the subjective assessment alone.

→ connects to [[Structural Interdependence]], [[Game Theory]], [[Coordination Without Convergence]], [[Ruth Chang]], [[Richard Bradley]]
